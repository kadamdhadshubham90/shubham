...
public class WebSearch {
    ...
    public void search(String startUrl) throws InterruptedException {
        ...
        this.searchPage(startUrl, executor);
        executor.shutdown();
        executor.awaitTermination(10, TimeUnit.MINUTES);
    }

    private void searchPage(String url, ExecutorService executor) {
        if (visitedLinks.add(url)) {
            executor.submit(() -> {
                try {
                    String content = this.fetchPageContent(url);
                    List<String> links = this.extractLinks(content, this.getDomain(url));
                    for (String link : links) {
                        this.searchPage(link, executor);
                    }
                } catch (Exception e) {
